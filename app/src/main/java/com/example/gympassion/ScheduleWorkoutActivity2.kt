...
import com.example.gympassion.databinding.ActivityScheduleWorkout2Binding
...
class ScheduleWorkoutActivity2 : AppCompatActivity() {
    private lateinit var binding: ActivityScheduleWorkout2Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityScheduleWorkout2Binding.inflate(layoutInflater)
            ...
            db.collection("users")
                ...
                .addOnSuccessListener { documents ->
                    for (document in documents) {
                        val days = document.data as Map<String, Boolean>
                        for (day in days) {
                            selectedDays[day.key] = day.value
                        }
                    }
                    // Teraz zaktualizuj widok na podstawie wybranych dni
                    daysNames.forEachIndexed { index, day ->
                        if (selectedDays[day] == true) {
                            daysCards2[index].setBackgroundColor(ContextCompat.getColor(this, R.color.colorOnClick))
                        } else {
                            daysCards2[index].setBackgroundColor(ContextCompat.getColor(this, R.color.colorOnRelease))
                        }
                    }

                    // Po odczycie dni, odczytujemy godziny
                    db.collection("users")
                        .document(userId)
                        ...
                        .addOnSuccessListener { documents ->
                            for (document in documents) {
                                val hours = document.data as Map<String, String>
                                for (hour in hours) {
                                    selectedHours[hour.key] = hour.value
                                }
                            }
                            // Teraz zaktualizuj widok na podstawie wybranych godzin
                            val hourArray = resources.getStringArray(R.array.workout_hours)
                            daysNames.forEachIndexed { index, day ->
                                if (selectedDays[day] == true) {
                                    val selectedHour = selectedHours[day]
                                    if (selectedHour != null) {
                                        val hourIndex = hourArray.indexOf(selectedHour)
                                        daysSpinners[index].setSelection(hourIndex)
                                    }
                                }
                            }
                        }
                        .addOnFailureListener { exception ->
                            Log.e("ScheduleWorkoutActivity2", "Error getting selected hours: ", exception)
                        }
                }
                .addOnFailureListener { exception ->
                    Log.e("ScheduleWorkoutActivity2", "Error getting selected days: ", exception)
                }
        }
    ...
    override fun onBackPressed() {
        ...
        builder.setPositiveButton("Tak") { dialog, id ->
            saveSelectedDays()  // saveSelectedDays() method is called before navigating back.
        }

        builder.setNegativeButton("Nie") { dialog, id ->
            super.onBackPressed()  // Navigates back without saving.
        }
    ...
    private fun deleteOldDocuments(collection: CollectionReference, onComplete: () -> Unit) {
        collection.get().addOnSuccessListener { documents ->
            val deleteTasks = documents.map { it.reference.delete() }
            Tasks.whenAllComplete(deleteTasks).addOnCompleteListener { onComplete() }
        }
    }

    private fun saveSelectedDays() {
        Log.d("ScheduleWorkoutActivity2", "Saving selected days")
        ...
        val userId = auth.currentUser?.uid
        if (userId != null) {
            Log.d("ScheduleWorkoutActivity2", "User ID: $userId")

            val selectedDaysCollection = db.collection("users").document(userId).collection("selectedDays")
            deleteOldDocuments(selectedDaysCollection) {
                selectedDaysCollection.document("schedule").set(selectedDays)
                    .addOnSuccessListener {
                        Log.d("ScheduleWorkoutActivity2", "Selected days saved")

                        val selectedHoursCollection = db.collection("users").document(userId).collection("selectedHours")
                        deleteOldDocuments(selectedHoursCollection) {
                            selectedHoursCollection.document("schedule").set(selectedHours)
                                .addOnSuccessListener {
                                    Log.d("ScheduleWorkoutActivity2", "Selected hours saved")

                                    val intentSettings = Intent(this, SettingsActivity::class.java)
                                    startActivity(intentSettings)
                                }
                                .addOnFailureListener { e ->
                                    Log.w("ScheduleWorkoutActivity2", "Error adding selected hours", e)
                                }
                        }
                    }
                    .addOnFailureListener { e ->
                        Log.w("ScheduleWorkoutActivity2", "Error adding selected days", e)
                    }
            }
        } else {
            Log.e("ScheduleWorkoutActivity2", "User ID is null")
        }
    }
